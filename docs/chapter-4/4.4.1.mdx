...
<Solution exercise="4.53">

```javascript
// 1. supervisionados por Ben
supervisor($person, list("Bitdiddle", "Ben"))

// 2. nomes e cargos da contabilidade
job($person, pair("accounting", $type))

// 3. nomes e endereços de quem mora em Slumerville
address($person, pair("Slumerville", $rest))
```

O segredo dos itens 2 e 3 é casar **o prefixo** da lista com `pair`: qualquer cargo cuja lista comece com `"accounting"`, qualquer endereço cuja lista comece com `"Slumerville"`.
...
<Solution exercise="4.54">

```javascript
// 1. supervisionados por Ben, com endereços
and(supervisor($person, list("Bitdiddle", "Ben")),
    address($person, $address))

// 2. salário menor que o de Ben, com os dois salários
and(salary(list("Bitdiddle", "Ben"), $ben_salary),
    and(salary($person, $amount),
        javascript_predicate($amount < $ben_salary)))

// 3. supervisionados por alguém fora da computação,
//    com nome e cargo do supervisor
and(supervisor($person, $boss),
    and(not(job($boss, pair("computer", $type))),
        job($boss, $boss_job)))
```

No item 2, a ordem importa: o salário de Ben precisa estar vinculado **antes** do `javascript_predicate` — filtros só funcionam sobre variáveis já instanciadas. No item 3, o `not` funciona como filtro pela mesma razão: `$boss` já chega vinculado pelo `supervisor`.
...
<Solution exercise="4.55">

```javascript
rule(can_replace($person_1, $person_2),
     and(job($person_1, $job_1),
         and(job($person_2, $job_2),
             and(or(same($job_1, $job_2),
                    can_do_job($job_1, $job_2)),
                 not(same($person_1, $person_2))))))

// 1. quem pode substituir Cy D. Fect
can_replace($person, list("Fect", "Cy", "D"))

// 2. quem pode substituir alguém que ganha mais, com os salários
and(can_replace($person_1, $person_2),
    and(salary($person_1, $salary_1),
        and(salary($person_2, $salary_2),
            javascript_predicate($salary_1 < $salary_2))))
```

(com a regra auxiliar `rule(same($x, $x))` do texto). A consulta 1 encontra Alyssa — programadores podem fazer o trabalho de técnicos; a 2 revela a injustiça de que ela ganha menos que Cy podendo substituí-lo.
...
<Solution exercise="4.56">

```javascript
rule(big_shot($person, $division),
     and(job($person, pair($division, $rest)),
         not(and(supervisor($person, $boss),
                 job($boss, pair($division, $boss_rest))))))
```

Uma pessoa é "grande chefe" na divisão se trabalha nela e **não** existe supervisor dela que também trabalhe nela — o que cobre tanto quem não tem supervisor nenhum (Oliver Warbucks) quanto quem responde a alguém de outra divisão (Ben, cujo chefe é da administração).
...
<Solution exercise="4.57">

**1. Reuniões de sexta-feira:**

```javascript
meeting($division, pair("Friday", $time))
```
...
```javascript
rule(meeting_time($person, $day_and_time),
     or(meeting("whole-company", $day_and_time),
        and(job($person, pair($division, $rest)),
            meeting($division, $day_and_time))))
```

As reuniões de alguém são as da empresa inteira **ou** as da divisão descoberta pelo seu cargo.

**3. As reuniões de Alyssa na quarta:**

```javascript
meeting_time(list("Hacker", "Alyssa", "P"),
             pair("Wednesday", $time))
```

que encontra duas: a da divisão de computação (3pm) e a da empresa toda (4pm).
...
<Solution exercise="4.58">

Cada par aparece **duas vezes** — `(Alyssa, Cy)` e também `(Cy, Alyssa)` — porque a regra `lives_near` é simétrica: o casamento de padrões encontra cada dupla nas duas ordens de vinculação das variáveis.

Para listar cada par só uma vez, quebre a simetria impondo uma **ordem** entre os dois nomes — por exemplo, exigir que o primeiro venha lexicograficamente antes do segundo:

```javascript
and(lives_near($person_1, $person_2),
    javascript_predicate(stringify($person_1) < stringify($person_2)))
```

Qualquer critério total e antissimétrico serve; a string da representação é o mais simples disponível no sistema. (Sem um predicado assim não há como fazê-lo só com padrões: o casador não tem noção de "menor que".)
...
<Solution exercise="4.59">

**Primeira consulta** — `next_to_in($x, $y, list(1, list(2, 3), 4))`:
...
os dois pares adjacentes da lista — a sublista `list(2, 3)` é um *elemento* como outro qualquer.

**Segunda consulta** — `next_to_in($x, 1, list(2, 1, 3, 1))`: quem vem imediatamente **antes** de um 1:
...
A primeira regra casa o começo da lista; a segunda desce recursivamente pela cauda — juntas, enumeram todas as adjacências.
...
<Solution exercise="4.60">

```javascript
rule(last_pair(list($x), list($x)))
rule(last_pair(pair($u, $v), $z),
     last_pair($v, $z))
```

Base: o último par de uma lista unitária é ela mesma. Passo: o último par de `pair($u, $v)` é o último par de `$v`.

- `last_pair(list(3), $x)` → `$x = list(3)`
- `last_pair(list(1, 2, 3), $x)` → `$x = list(3)`
- `last_pair(list(2, $x), list(3))` → `$x = 3` — as regras funcionam "de trás para frente"!

**E `last_pair($x, list(3))`?** O sistema encontra `list(3)`, `list($u, 3)`, `list($u, $v, 3)`, … — **infinitas** respostas, cada vez com mais variáveis não vinculadas na frente. A consulta não termina de enumerar; regras lógicas não têm direção preferida, e perguntas "abertas demais" geram espaços de busca infinitos.
...
A primeira é a definição direta de neto; a segunda estende a relação `son`: os filhos da esposa contam como filhos do marido — e, crucialmente, a regra *alimenta a si mesma e à de neto*, porque o `son($f, $s)` dentro de `grandson` também pode ser satisfeito via esposa.

- `grandson("Cain", $x)` → Irad
- `son("Lamech", $x)` → Jabal e Jubal (via Ada)
- `grandson("Methushael", $x)` → Jabal e Jubal (Lamech é filho de Methushael; os filhos de Lamech vêm pela regra da esposa)
