...
[^2]: O método `apply` do JavaScript espera os argumentos da função em um *vetor*. (Vetores são chamados de "arrays" em JavaScript.) Assim, o `arglist` é transformado em um vetor—aqui usando um loop while (veja o exercício 4.7):
...
A `map` de Eva é definida **dentro** da linguagem avaliada: quando ela passa uma função para `map`, passa um *objeto-função do avaliador* (uma lista etiquetada com parâmetros, corpo e ambiente), e é o próprio avaliador que a aplica, via seu `apply`.

O `map` de Louis é a função do sistema hospedeiro instalada como primitiva. Quando o avaliador aplica uma primitiva, ele chama a função JavaScript subjacente passando os argumentos *do avaliador* — e aí está o desastre: o `map` do sistema espera receber uma **função JavaScript de verdade** como primeiro argumento, mas recebe a representação interna do avaliador (a lista `list("compound_function", parâmetros, corpo, ambiente)`). O `map` do sistema tenta invocá-la e falha — a estrutura de dados não é invocável.

A fronteira entre os dois mundos só é atravessada com tradução: primitivas funcionam quando operam sobre valores que as duas camadas representam igual (números, strings, pares). Funções são exatamente o ponto em que as representações divergem — o motivo de o avaliador ter o *seu* `apply`.
