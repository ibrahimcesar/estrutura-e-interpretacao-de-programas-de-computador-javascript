...
Em uma linguagem puramente de ordem aplicativa, todas as funções são estritas em cada argumento. Em uma linguagem puramente de ordem normal, todas as funções compostas são não estritas em cada argumento, e as funções primitivas podem ser estritas ou não estritas. Existem também linguagens (veja o exercício 4.29) que dão aos programadores controle detalhado sobre a rigorosidade das funções que eles definem.
