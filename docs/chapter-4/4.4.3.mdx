...
<Solution exercise="4.62">

Compare com a regra original da seção 4.4.1: lá, o `and` recursivo era
...
— `supervisor` primeiro. Louis inverteu a ordem dos conjuntos. A diferença é fatal por causa de **como o `and` processa**: o primeiro conjunto roda com o frame corrente, e o segundo recebe os frames estendidos.

Na regra original, `supervisor($staff_person, $middle_manager)` roda primeiro com `$staff_person` vinculado (a Ben) — a base de dados responde com poucos `$middle_manager` concretos, e a recursão desce *ancorada* em pessoas reais, terminando quando chega a quem não tem supervisor.

Na regra de Louis, o primeiro conjunto é `outranked_by($middle_manager, $boss)` — com **ambas as variáveis livres**. Para respondê-lo, o sistema aplica a própria regra `outranked_by` de novo… cujo primeiro conjunto é, de novo, um `outranked_by` com variáveis livres, *idêntico ao anterior a menos de renomeação*. Nenhum fato da base de dados é consultado para ancorar a descida: a cadeia de aplicações de regra cresce para sempre, sem progresso. As respostas que DeWitt vê antes do travamento vêm do primeiro ramo do `or` (os `supervisor` diretos), que o entrelaçamento de streams ainda consegue entregar — o loop está no segundo ramo.
...
<Solution exercise="4.63">

O sistema não responde "quem é mandachuva" — ele enumera **derivações**. A regra
...
produz um resultado para **cada par** `($middle_manager, $x)` que a satisfaz. Oliver Warbucks é o chefe de Ben Bitdiddle e de Eben Scrooge; Ben supervisiona três pessoas (Alyssa, Cy e Lem) e Scrooge supervisiona uma (Robert Cratchet). São $3 + 1 = 4$ cadeias distintas `subordinado → gerente-do-meio → Warbucks`, logo quatro frames distintos — e quatro linhas `wheel(list("Warbucks", "Oliver"))`, uma por cadeia. Ben Bitdiddle aparece uma vez: só Louis → Alyssa → Ben.

Os frames são de fato diferentes (variam em `$middle_manager` e `$x`); é a *instanciação da consulta* — que só menciona `$who` — que os torna indistinguíveis na saída. O sistema imprime o padrão instanciado por frame, sem eliminar repetições.
...
onde `accumulation_function` podem ser coisas como `sum`, `average` ou `maximum`. Ben raciocina que deve ser moleza implementar isso. Ele simplesmente alimentará o padrão de consulta para `evaluate_query`. Isso produzirá um stream de frames. Ele então passará esse stream através de uma função de mapeamento que extrai o valor da variável designada de cada frame no stream e alimentará o stream resultante de valores para a função de acumulação. Assim que Ben completa a implementação e está prestes a testá-la, Cy passa por ali, ainda intrigado com o resultado da consulta `wheel` no exercício 4.63. Quando Cy mostra a Ben a resposta do sistema, Ben geme, "Oh, não, meu esquema simples de acumulação não vai funcionar!"

O que Ben acabou de perceber? Delineie um método que ele pode usar para salvar a situação.

<Solution exercise="4.64">

**O que Ben percebeu:** o stream de frames contém uma entrada por *derivação*, não por *fato*. Se somar os salários dos "wheels" da consulta de Cy, o salário de Warbucks entraria **quatro vezes**. Toda `accumulation_function` sobre o stream cru herda essa contagem inflada.

**O que não funciona como conserto:** eliminar valores repetidos da variável extraída. Duas pessoas diferentes podem legitimamente ter o mesmo salário — ambas devem contar. O problema não são valores iguais; são *derivações repetidas do mesmo fato*.

**O método que salva a situação:** deduplicar **frames equivalentes do ponto de vista do padrão** antes de extrair valores — instancie o padrão da consulta com cada frame (o que o exibidor de resultados já faz) e descarte instanciações repetidas:

```
frames → instancia padrão → filtra instâncias já vistas → extrai $variable → acumula
```

Com isso, as quatro derivações de `wheel(Warbucks)` colapsam numa só, mas dois programadores com salários iguais continuam sendo dois itens — pois suas instanciações diferem em `$x`. (Resta o custo: "já visto" exige memória das instâncias anteriores, e streams infinitos de resultados nunca terminam de acumular — a acumulação só faz sentido sobre consultas finitas.)
...
Invente uma maneira de instalar um detector de loop no sistema de consulta para evitar os tipos de loops simples ilustrados no texto e no exercício 4.62. A ideia geral é que o sistema deve manter algum tipo de histórico de sua cadeia atual de deduções e não deve começar a processar uma consulta na qual já está trabalhando. Descreva que tipo de informação (padrões e frames) está incluída neste histórico, e como a verificação deve ser feita. (Depois de estudar os detalhes da implementação do sistema de consulta na seção 4.4.4, você pode querer modificar o sistema para incluir seu detector de loop.)

<Solution exercise="4.65">

**O histórico:** uma lista — threading pela cadeia *corrente* de deduções, não global — de pares `(padrão da consulta, frame)` das aplicações de regra em andamento: no momento em que `apply_a_rule` vai processar o corpo de uma regra para responder uma consulta, registra o padrão *instanciado com as vinculações já conhecidas* do frame corrente.

**A verificação:** antes de iniciar uma nova aplicação de regra, instancie o padrão da nova consulta com seu frame e compare com cada entrada do histórico da cadeia. Se for **idêntico a menos de renomeação de variáveis livres** (a mesma pergunta, com os mesmos vinculados e liberdade nos mesmos lugares), o sistema está prestes a perguntar algo em cujo processamento já está no meio — corte o ramo, fazendo-o contribuir com o stream vazio.

**Por que isso é seguro para os loops simples:** no exercício 4.62, a recursão gera exatamente a mesma consulta `outranked_by($a, $boss)` com o mesmo (nenhum) vínculo — a comparação a pega na segunda aplicação. Idem para o par `married(Minnie, Mickey)` do texto.

**Por que a comparação precisa ser "a menos de renomeação":** cada aplicação de regra renomeia as variáveis da regra para evitar colisões; textualmente as consultas nunca repetem. E a igualdade deve exigir os *mesmos* vínculos: `outranked_by(Ben, $boss)` dentro de `outranked_by($a, $boss)` é progresso legítimo, não loop. O detector não elimina todos os não-términos possíveis (o exercício 4.62 lembra que isso é o problema da parada — exercício 4.15), mas mata os ciclos de pergunta-idêntica que dominam a prática.
...
<Solution exercise="4.66">

```javascript
assert(rule(reverse(null, null)));
assert(rule(reverse(pair($u, $v), $y),
            and(reverse($v, $rev_v),
                append_to_form($rev_v, list($u), $y))));
```

Lendo: o reverso da lista vazia é a vazia; o reverso de `u ⋅ v` é o reverso de `v` com `u` acrescentado ao final.

**Direcionalidade:** `reverse(list(1, 2, 3), $x)` funciona — a recursão desmonta o primeiro argumento, que está totalmente vinculado, e termina em `null`. Já `reverse($x, list(1, 2, 3))` trava: o subobjetivo `reverse($v, $rev_v)` roda com **ambas as variáveis livres**, e o sistema aplica a regra recursivamente sem nenhum dado para ancorá-la — o mesmo loop de variáveis livres do exercício 4.62. Invertendo a ordem dos conjuntos do `and` (primeiro `append_to_form`, que decompõe `$y`), a regra passa a responder a *segunda* consulta e a travar na primeira. Um único conjunto de regras não serve às duas direções com o avaliador atual; com o detector de loops do exercício 4.65 (ou avaliação justa dos conjuntos), as duas direções poderiam conviver — a beleza declarativa do "o quê" cobrando seu preço no "como".
...
Vamos modificar a base de dados e as regras do exercício 4.61 para adicionar "great" a uma relação de neto. Isso deve permitir que o sistema deduza que Irad é o bisneto de Adão, ou que Jabal e Jubal são os tataratataranetos de Adão.
...
<Solution exercise="4.67">

**1. A base reformulada** — um único tipo de asserção, com a relação como primeiro item:

```javascript
assert(related(list("son"), "Adam", "Cain"));
assert(related(list("son"), "Cain", "Enoch"));
assert(related(list("son"), "Enoch", "Irad"));
assert(related(list("son"), "Irad", "Mehujael"));
// … e as demais; a regra de neto vira:
assert(rule(related(list("grandson"), $x, $y),
            and(related(list("son"), $x, $z),
                related(list("son"), $z, $y))));
```

(As regras de "esposa⇒filho" da solução do exercício 4.61 são reescritas no mesmo formato.)

**2. Termina em `"grandson"`:**

```javascript
assert(rule(ends_in_grandson(list("grandson"))));
assert(rule(ends_in_grandson(pair($first, $rest)),
            ends_in_grandson($rest)));
```

**3. A regra do "great":**

```javascript
assert(rule(related(pair("great", $rel), $x, $y),
            and(ends_in_grandson($rel),
                related(list("son"), $x, $z),
                related($rel, $z, $y))));
```

**Por que funciona:** `(great ⋅ rel)(x, y)` vale quando `x` tem um filho `z` e `rel(z, y)` — cada `"great"` desloca a relação uma geração para baixo. Para Irad: `son(Adam, Cain)` e `grandson(Cain, Irad)` dão `related(list("great", "grandson"), "Adam", "Irad")` — Irad é bisneto de Adão ✓. Para Jabal e Jubal, quatro `"great"` empilhados descem as gerações até os filhos de Lamech. O `ends_in_grandson` mantém a recursão dentro da família de relações pretendida — sem ele, a regra tentaria dar significado a `pair("great", qualquer-coisa)` e reabriria a porta dos loops de variáveis livres.
