...
<Solution exercise="4.25">

As respostas, na ordem, com a explicação do estado de `count`:

- Após `const w = id(id(10));` → **`count` é 1**: a aplicação externa de `id` executa (é a expressão da declaração), incrementando `count`; mas seu argumento `id(10)` vira um *thunk* — a aplicação interna ainda não aconteceu.
- `w` → **10**: o driver força o valor de `w`; isso força o thunk, executando o `id` interno.
- `count` → **2**: o forçamento acima incrementou o contador.

O quebra-cabeça inteiro está em *quando* cada `id` roda: um na avaliação da declaração, o outro só quando alguém finalmente olha para `w`. Efeitos colaterais + preguiça = cronologia contraintuitiva — o tema dos exercícios seguintes.
...
<Solution exercise="4.26">

Force a expressão de função quando **ela própria é um argumento adiado**. Exemplo:
...
Dentro de `apply_twice`, o parâmetro `f` está vinculado a um *thunk* (todo argumento chega adiado). Na aplicação `f(x)`, a expressão de função é o nome `f`, cujo valor é o thunk — se o avaliador usasse `evaluate` em vez de `actual_value`, tentaria aplicar **um thunk** e falharia ("unknown procedure type"). O `actual_value` desfaz o embrulho e revela a função de verdade antes do `apply`.

Qualquer função de ordem superior dispara o problema — ou seja, praticamente qualquer programa interessante deste livro.

</Solution>

### Exercício 4.27

Exiba um programa que você esperaria que executasse muito mais lentamente sem memorização do que com memorização. Além disso, considere as seguintes interações, onde a função `id` é definida como no exercício 4.25 e `count` começa em 0:
...
**Programa muito mais lento sem memoização:** qualquer um com recursão em árvore sobre um argumento adiado —
...
Sem memoização, cada uso de `n` dentro de `fib` reavalia o thunk do argumento, e os thunks se aninham a cada nível: o custo explode exponencialmente além do já exponencial `fib`. Com memoização, cada thunk é forçado uma vez.

**As interações com `id`:**

Com memoização: `square(id(10))` responde **100** e `count` responde **1** — o thunk de `x` é forçado no primeiro uso dentro de `x * x` e o segundo uso reaproveita o valor lembrado.

Sem memoização: `square(id(10))` responde **100**, mas `count` responde **2** — `x * x` força o thunk duas vezes, e `id` roda de novo a cada forçamento.
...
**a. Por que o medo de Cy não se concretiza no caso comum:** numa sequência, cada declaração é *avaliada* — e avaliar uma declaração de expressão que é uma **aplicação** executa a função (só os argumentos ficam adiados). `display(x);` no meio de uma sequência roda o `display` (primitiva estrita, que força seu argumento). O caso do `for_each` do texto funciona exatamente por isso: o corpo é uma aplicação de primitiva.

**b. As exceções reais:** efeitos escondidos *dentro de um thunk que ninguém força*. O exemplo clássico:

```javascript
function f(e) {
    e;            // ← declaração de expressão que é só um nome
    return "pronto";
}
f(x = 5);
```

O argumento `x = 5` chega como thunk; a declaração `e;` avalia o nome `e`… para o próprio thunk, **sem forçá-lo** — a atribuição nunca acontece, e `x` continua com o valor antigo. Qualquer expressão-declaração cujo valor é um thunk não-forçado (um nome, uma condicional que devolve argumento adiado…) engole seus efeitos.

**c. O que fazer:** as opções são (1) manter como está — sequências não forçam, e efeitos em thunks se perdem silenciosamente; (2) forçar toda declaração não-final com `actual_value` — sequências recuperam a leitura imperativa, ao custo de forçar coisas possivelmente caras ou infinitas; (3) distinguir sintaticamente. Numa linguagem *preguiçosa com atribuição*, a opção 2 é a menos traiçoeira: quem escreve uma declaração de expressão no meio de uma sequência quase sempre quer o efeito. Mas a resposta honesta do exercício é a do livro: misturar preguiça com efeitos colaterais é intrinsecamente escorregadio — os capítulos 3 e 4 puxam em direções opostas.
...
<Solution exercise="4.29">

Três mudanças coordenadas:

**1. Sintaxe.** O parser marca cada parâmetro com seu modo; a lista de parâmetros vira pares `(nome, modo)`, com `"strict"` como padrão e `"lazy"` para os anotados.

**2. `apply`.** Em vez de adiar todos os argumentos, decida por parâmetro:

```javascript
function apply(fun, arg_exps, env) {
    …
    const params = function_parameters(fun);
    const args = map_2((param, exp) =>
                           parameter_mode(param) === "lazy"
                           ? delay_it(exp, env)
                           : actual_value(exp, env),
                       params, arg_exps);
    return … extend_environment(map(parameter_name, params), args, …);
}
```

**3. Busca de nomes.** `lookup` continua forçando thunks quando o valor é usado (ou o corpo usa `actual_value` nos pontos de força, como antes) — parâmetros estritos já chegam prontos e não pagam nada.

Com isso, `function f(a, lazy b, c, lazy d)` avalia `a` e `c` na chamada e adia `b` e `d` — o `unless` do exercício 4.23 vira simplesmente `function unless(cond, lazy usual, lazy exceptional)`, e o resto da linguagem permanece estrito e previsível. É o meio-termo entre os mundos: preguiça como *anotação deliberada*, não como regime global.
