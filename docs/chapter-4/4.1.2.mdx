...
O truque é **sequenciar explicitamente** com constantes locais — a ordem de avaliação de declarações `const` numa sequência é garantida, não importa o que o JavaScript subjacente faça com argumentos:
...
function list_of_values(exps, env) {
    if (is_null(exps)) {
        ...
        const first = evaluate(first_operand(exps), env);
        const rest = list_of_values(rest_operands(exps), env);
        return pair(first, rest);
    }
}

// direita → esquerda: basta trocar a ordem das duas constantes
function list_of_values_rl(exps, env) {
    if (is_null(exps)) {
        return null;
    } else {
        const rest = list_of_values_rl(rest_operands(exps), env);
        const first = evaluate(first_operand(exps), env);
        return pair(first, rest);
    }
}
...
Na versão original, `pair(evaluate(...), list_of_values(...))` deixava a decisão para a linguagem hospedeira; agora cada `evaluate` acontece numa declaração própria, e declarações executam em ordem.
...
A mudança avalia o *símbolo do operador* no ambiente — ou seja, `+` passa a ser tratado como um **nome** a ser procurado, e a aplicação usa a função encontrada. Para isso funcionar, o ambiente global precisa ter vínculos para os operadores:

```javascript
// no ambiente global:
define_variable("+", make_primitive_function((x, y) => x + y), the_global_environment);
define_variable("-", make_primitive_function((x, y) => x - y), the_global_environment);
define_variable("*", make_primitive_function((x, y) => x * y), the_global_environment);
// … e assim por diante para os demais operadores
```

**O ganho:** operadores viram cidadãos comuns — o programa avaliado poderia até redefinir `+` (útil, por exemplo, para a aritmética genérica do capítulo 2). **O custo:** perde-se a garantia de que `+` significa adição, e o avaliador fica preso a nomes de operador válidos como identificadores no ambiente — uma troca entre uniformidade e previsibilidade.
...
**Como formas derivadas** — a transformação para expressões condicionais preserva o curto-circuito, porque o ramo não escolhido nunca é avaliado:

```javascript
// a && b  ⟶  a ? b : false     (mais precisamente: guarda o valor do 1º)
// a || b  ⟶  a ? true : b
function and_to_conditional(component) {
    return make_conditional_expression(
               and_first(component),
               and_second(component),
               make_literal(false));
}
function or_to_conditional(component) {
    return make_conditional_expression(
               or_first(component),
               make_literal(true),
               or_second(component));
...
com os casos despachados em `evaluate` avaliando a transformação. (Para reproduzir fielmente a semântica descrita — devolver *o valor do operando*, não `true`/`false` — guarda-se o primeiro operando numa constante local antes do teste.)

**Como funções derivadas não dá**: uma aplicação `and(a, b)` avaliaria **os dois** argumentos antes de aplicar — e o ponto inteiro de `&&`/`||` é não avaliar o segundo operando quando o primeiro decide. Curto-circuito exige forma sintática (ou argumentos atrasados, o tema da seção 4.2).
...
A tradução de `switch_to_cond` encadeia expressões condicionais, agrupando os `case` que "caem" juntos (como `case 2: case 3:`) com `||` de igualdades:
...
Em geral: cada grupo de rótulos até um `break` vira um ramo, com predicado `chave === r1 || chave === r2 || …`, e o `default` vira a alternativa final (ou `undefined` se ausente). Para não reavaliar a expressão da chave a cada teste, a tradução a captura primeiro numa constante com nome não convencional:

```javascript
{
    const "🔑chave" = x;   // nome impossível de colidir com o usuário
    "🔑chave" === 1 ? f() : "🔑chave" === 2 || "🔑chave" === 3 ? g() : h();
}
```

A restrição importante: essa tradução cobre `switch` **com** `break` em cada grupo — o *fall-through* sem `break` do JavaScript pleno não é expressável como condicional pura (exigiria duplicar caudas de casos).
...
A tradução clássica: o laço vira uma função recursiva de cauda com nome não convencional:
...
isto é, se o predicado vale, avalia-se o corpo e chama-se o laço de novo; senão termina com `undefined`. Como a chamada recursiva está em posição de cauda, um avaliador tail-recursive itera em espaço constante.

**`break` e `continue`** entram como *continuações* passadas ao corpo — o corpo é traduzido para uma função que recebe duas funções com nomes não convencionais:

```javascript
{
    function "⟳while"() {
        return predicate
               ? "⟳corpo"(() => undefined,      // break: sai sem repetir
                          () => "⟳while"())     // continue: repete o laço
               : undefined;
    }
    "⟳while"();
}
...
Na tradução do corpo, `break;` vira `return "⟳break"();` e `continue;` vira `return "⟳continue"();`; se o corpo terminar normalmente, a tradução chama `"⟳continue"()` por conta própria. Sair do laço é, literalmente, *chamar a continuação de saída* — uma amostra do estilo que o avaliador `amb` da seção 4.3 usará em escala.
...
**1. Os quatro programas:**

| Programa | Valor | Por quê |
|---|---|---|
| `1; 2; 3;` | `3` | última declaração value-producing |
| `1; { if (true) {} else { 2; } }` | `undefined` | o bloco é value-producing, mas o ramo executado (`{}`) não produz valor |
| `1; const x = 2;` | `1` | a declaração é non-value-producing; a última produtora é `1;` |
| `1; { let x = 2; { x = x + 3; } }` | `5` | a atribuição é uma expressão; seu valor (5) sobe pelo bloco |

(Confira num console JavaScript — a especificação do exercício é exatamente a *completion value semantics* da linguagem, a mesma que faz o eco dos playgrounds deste site funcionar.)

**2. A modificação:** `evaluate_sequence` para programas de topo passa a acompanhar o último valor produzido, distinguindo estaticamente os tipos de declaração:

```javascript
function eval_program_sequence(stmts, env) {
    let value = undefined;
    let stmt_list = stmts;
    while (!is_null(stmt_list)) {
        const stmt = head(stmt_list);
        const result = evaluate(stmt, env);
        if (is_value_producing(stmt)) {   // expressão, condicional,
            value = result;               // bloco value-producing…
        }
        stmt_list = tail(stmt_list);
    }
    return value;
}
```

com `is_value_producing` definida por inspeção sintática: expressões e condicionais sim; declarações não; blocos e sequências, recursivamente conforme seus componentes. A avaliação de corpos de função permanece como era — lá quem manda são as declarações `return`.
