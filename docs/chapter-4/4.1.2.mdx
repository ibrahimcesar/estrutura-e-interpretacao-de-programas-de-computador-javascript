...
Forms sintáticas que podem ser implementados como transformações sintáticas são chamados de *componentes derivados*. Traduzir uma declaração `switch` (exercício adicional abaixo) em expressões condicionais aninhadas é um exemplo de componente derivado.

## Exercício adicional: combinações de operadores

> **Nota da tradução:** este exercício é um adicional desta edição brasileira — não faz parte da numeração da edição JS oficial.
...
## Exercício adicional: declarações `switch`
