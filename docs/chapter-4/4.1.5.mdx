...
Suponha que `halts` exista e avalie `strange(strange)`. Dentro da chamada, `halts(strange, strange)` responde a pergunta: "`strange(strange)` para?" — exatamente a expressão que estamos avaliando. Siga os dois casos:

- **Se `halts(strange, strange)` retorna `true`** ("`strange(strange)` para"), então `strange` executa o ramo `run_forever()` — e `strange(strange)` **não para**. `halts` mentiu.
- **Se retorna `false`** ("`strange(strange)` não para"), então `strange` retorna `"halted"` imediatamente — e `strange(strange)` **para**. `halts` mentiu de novo.

Todo caso leva a `halts` contradizendo a si mesma; como `halts` foi a única hipótese, ela não pode existir. É o argumento diagonal de Cantor vestido de programa: `strange` é construída *a partir de* `halts` precisamente para fazer o oposto do que `halts` prevê sobre ela mesma.

**Por que nenhum truque escapa:** o raciocínio não usa nada sobre *como* `halts` funciona — nem que ela só recebe a função como caixa-preta (a nota de rodapé estende o argumento para `halts` com acesso ao texto-fonte e ao ambiente). Qualquer suposto analisador estático perfeito de término cai na mesma armadilha, porque `strange` pode ser construída mecanicamente a partir dele.

**Conexão com a seção:** é a contrapartida negativa de "dados como programas" — a mesma universalidade que permite a `evaluate` rodar qualquer programa impede que qualquer programa preveja o comportamento de todos os programas. A auto-aplicação `strange(strange)` é prima da auto-aplicação do exercício 4.18 — lá construtiva, aqui destrutiva.
