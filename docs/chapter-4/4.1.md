...
```mermaid
flowchart LR
    Eval["evaluate<br/><i>(expressões/declarações<br/>+ ambiente)</i>"]
    Apply["apply<br/><i>(função<br/>+ argumentos)</i>"]

    Eval -->|"aplicação de função"| Apply
    Apply -->|"corpo da função<br/>+ ambiente"| Eval

    Eval -.->|"valores primitivos,<br/>lookup de nomes"| Result["valores"]
    Apply -.->|"funções primitivas"| Result

    style Eval fill:#e1f5ff,stroke:#0066cc,stroke-width:3px
    style Apply fill:#ffe1f5,stroke:#cc0066,stroke-width:3px
    style Result fill:#e1ffe1,stroke:#006600,stroke-width:2px
```
