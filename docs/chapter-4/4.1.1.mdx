...
O truque é **sequenciar com declarações**: a ordem de avaliação de declarações `const` consecutivas é garantida pela semântica do JavaScript, não importa em que ordem os argumentos de `pair` sejam avaliados:

```javascript
// esquerda → direita
function list_of_values_ltr(exps, env) {
    if (is_null(exps)) {
        return null;
    } else {
        const first = evaluate(first_operand(exps), env);
        const rest = list_of_values_ltr(rest_operands(exps), env);
        return pair(first, rest);
    }
}

// direita → esquerda
function list_of_values_rtl(exps, env) {
    if (is_null(exps)) {
        return null;
    } else {
        const rest = list_of_values_rtl(rest_operands(exps), env);
        const first = evaluate(first_operand(exps), env);
        return pair(first, rest);
    }
}
...
**Por que funciona:** em `pair(evaluate(...), list_of_values(...))`, quem decide a ordem é o JavaScript hospedeiro. Ao dar nome a cada resultado intermediário numa `const`, nós — e não o hospedeiro — fixamos a sequência: a primeira declaração é avaliada por completo antes da segunda começar.

**Armadilha:** a diferença só é observável quando as expressões de argumento têm efeitos (atribuições, `display`). É por isso que o capítulo 3 precisou vir antes deste: sem estado, as duas ordens são indistinguíveis.
