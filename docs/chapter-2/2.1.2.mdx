...
<CodePlayground
  code={`function make_point(x, y) { return pair(x, y); }
function x_point(p) { return head(p); }
function y_point(p) { return tail(p); }

function make_segment(start, end) { return pair(start, end); }
function start_segment(s) { return head(s); }
function end_segment(s) { return tail(s); }
...
print_point(midpoint_segment(
    make_segment(make_point(0, 0), make_point(4, 6))));`}
  height={520}
  showLineNumbers={false}
/>

Dois níveis de abstração empilhados: segmentos sobre pontos, pontos sobre pares. `midpoint_segment` só fala a linguagem de segmentos e pontos — nunca menciona `pair`, `head` ou `tail`.
...
**Ideia.** Escreva `perimeter` e `area` em função de apenas duas medidas — `width_rect` e `height_rect` — e faça de *cada representação* a responsável por fornecê-las.

<CodePlayground
  code={`// representação 1: dois cantos opostos
...
// funções que dependem SÓ da barreira width/height:
...
const r = make_rect(make_point(1, 1), make_point(5, 4));
display(perimeter(r));
area(r);`}
  height={560}
  showLineNumbers={false}
...
**A segunda representação** — canto + largura + altura — troca só o construtor e os dois seletores:

```javascript
function make_rect(corner, width, height) {
    return pair(corner, pair(width, height));
}
function width_rect(r) { return head(tail(r)); }
function height_rect(r) { return tail(tail(r)); }
```

`perimeter` e `area` não mudam uma vírgula: é a barreira de abstração fazendo o trabalho — quem consome retângulos não sabe (nem quer saber) como eles são feitos por dentro.
