...
**Ideia.** Normalize tudo de uma vez: reduza pelo `gcd` e force o sinal para o numerador, testando só o denominador.
...
function make_rat(n, d) {
    const g = gcd(math_abs(n), math_abs(d));
    return d < 0
           ? pair(-n / g, -d / g)
           : pair(n / g, d / g);
}
...
**Por que funciona.** Se `d < 0`, negamos os dois termos — isso cobre tanto `6/-9` (vira `-2/3`) quanto `-6/-9` (vira `2/3`); se `d > 0`, o sinal de `n` já está certo. Como o `gcd` recebe valores absolutos, a redução não interfere na normalização.
