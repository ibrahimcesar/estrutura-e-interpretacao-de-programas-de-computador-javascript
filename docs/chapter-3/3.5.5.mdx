...
<Solution exercise="3.81">

Sem atribuição: o estado (o valor corrente do gerador) viaja como argumento na própria recursão do stream:

<CodePlayground
  code={`function random_numbers_from(init, requests) {
    return is_null(requests)
           ? null
           : head(requests) === "generate"
           ? make_next(rand_update(init), stream_tail(requests))
           : make_next(head(tail(head(requests))),
                       stream_tail(requests));
}
function make_next(value, rest) {
    return pair(value,
                memo(() => random_numbers_from(value, rest)));
}

// pedidos: gerar, gerar, reset para 42, gerar, gerar, reset 42, gerar
const requests =
...
first_n(random_numbers_from(random_init, requests), 7);`}
...
Um pedido `"generate"` produz `rand_update` do valor anterior; um pedido `list("reset", v)` faz o stream continuar a partir de `v`. Repare nas saídas após os dois resets para 42: as subsequências coincidem — reprodutibilidade sem nenhuma variável mutável, porque o "estado" é só o argumento que flui pela recursão.
...
function monte_carlo_stream(experiment_stream, passed, failed) {
    function next(passed, failed) {
        return pair(passed / (passed + failed),
                    memo(() => monte_carlo_stream(
                        stream_tail(experiment_stream),
                        passed, failed)));
    }
    return head(experiment_stream)
           ? next(passed + 1, failed)
...
function estimate_integral_stream(P, x1, x2, y1, y2) {
    function experiments() {
        return pair(P(random_in_range(x1, x2),
                      random_in_range(y1, y2)),
                    () => experiments());
    }
    return scale_stream(
        monte_carlo_stream(experiments(), 0, 0),
        (x2 - x1) * (y2 - y1));
}

const pi_stream = estimate_integral_stream(
    (x, y) => x * x + y * y <= 1, -1, 1, -1, 1);

stream_ref(pi_stream, 10000);`}
  hiddenCode={`function memo(fun) {
    let already_run = false;
    let result;
    return () => {
...
Em vez de um número fixo de tentativas, o resultado é o **stream de estimativas**: o elemento $n$ usa $n+1$ experimentos. `stream_ref(pi_stream, 10000)` dá $\pi$ com uma casa ou duas; puxe mais fundo e a estimativa refina — o leitor decide quanta precisão quer *depois* de o stream existir, que é exatamente o luxo que a avaliação preguiçosa compra.
