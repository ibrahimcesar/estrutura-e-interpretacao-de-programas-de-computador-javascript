...
`const acc = make_account(50);` cria o ambiente `E1` com `balance = 50` e os vínculos de `withdraw`, `deposit` e `dispatch` — os três objetos-função apontam para `E1` como ambiente de criação. `acc` é o `dispatch`.

`acc("deposit")(40)`:

1. Aplicar `dispatch` cria `E2: m = "deposit"` → `E1`; devolve o objeto `deposit`.
2. Aplicar `deposit` cria `E3: amount = 40` → `E1` (o ambiente de *criação* de `deposit`, não `E2`!); a atribuição encontra `balance` em `E1` e o muda para 90.

`acc("withdraw")(60)` repete o padrão com dois novos ambientes e deixa `balance = 30` em `E1`.

**Onde está o estado local de `acc`?** Inteiramente em `E1` — o ambiente criado pela chamada `make_account(50)`.

**`acc` e `acc2` como se mantêm distintos?** `acc2 = make_account(100)` cria um `E1'` separado, com seu próprio `balance` e suas próprias funções `withdraw`/`deposit`/`dispatch` fechando sobre `E1'`. Nada é compartilhado entre as duas contas — exceto, numa implementação eficiente, o *código* (o texto das funções), já que só os ambientes precisam diferir.
