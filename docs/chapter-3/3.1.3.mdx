...
const paul_acc = make_account(100);`}
  hiddenCode={`function make_account(balance) {
...
<CodePlayground
  ...
  checks={[{expression: 'paul_acc("rosebud", "withdraw")(10)', expected: '90'}]}
  hiddenCode={`function make_account(balance, password) {
    function withdraw(amount) {
        if (balance >= amount) {
            balance = balance - amount;
            return balance;
...
**Ideia.** A conta conjunta é só um *tradutor de senhas*: se a nova senha confere, delega à conta original com a senha original.

<CodePlayground
  code={`function make_joint(account, password, new_password) {
    return (p, m) => p === new_password
                     ? account(password, m)
                     : () => "Incorrect password";
}
...
display(paul_acc("rosebud", "withdraw")(10));
...
**Por que funciona.** `paul_acc` e `peter_acc` compartilham o MESMO objeto de conta — o saque de Paul deixa 90, e o saque seguinte de Peter parte dos 90 e deixa 80. É a "conta conjunta" da discussão de identidade desta seção, construída sem tocar em `make_account`.
...
**Ideia.** Precisamos de uma `f` cujo resultado dependa da *ordem* das chamadas — ou seja, com estado. Uma que devolve o argumento na primeira chamada e 0 nas seguintes resolve:
...
- **Esquerda → direita:** `f(0)` responde 0 (primeira chamada) e `f(1)` responde 0 → soma **0**.
- **Direita → esquerda:** `f(1)` responde 1 e `f(0)` responde 0 → soma **1**.

O resultado 0 que você vê acima confirma que o JavaScript avalia operandos da esquerda para a direita (isso é fixado pela especificação da linguagem). A moral do exercício: com atribuição, até o `+` deixa de ser inocente — expressões passam a ter *história*.
