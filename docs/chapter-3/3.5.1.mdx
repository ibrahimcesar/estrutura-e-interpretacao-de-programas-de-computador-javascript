...
<Solution exercise="3.50">

<CodePlayground
  code={`function stream_map_2(f, s1, s2) {
    return is_null(s1) || is_null(s2)
           ? null
           : pair(f(head(s1), head(s2)),
                  () => stream_map_2(f, stream_tail(s1),
                                        stream_tail(s2)));
}
...
first_n(add_streams(integers, integers), 5);`}
...
A única diferença da versão otimizada é o `memo(...)` em volta da cauda adiada: a primeira materialização é lembrada e as seguintes não recomputam nada.
...
<Solution exercise="3.51">

**Com `stream_map` (sem memoização):**

- Na construção de `x`, apenas **`0`** é exibido — `stream_map` aplica `display` à cabeça imediatamente; o resto fica adiado.
- `stream_ref(x, 5)` materializa cinco caudas, exibindo **1 2 3 4 5**, e o eco do REPL é `5` (o `display` devolve seu argumento).
- `stream_ref(x, 7)` **recomeça do zero**: as caudas não memoizadas recomputam tudo, exibindo **1 2 3 4 5 6 7** de novo, com eco `7`.

**Com `stream_map_optimized`:** a construção exibe `0`, `stream_ref(x, 5)` exibe `1 2 3 4 5` — e `stream_ref(x, 7)` exibe apenas **`6 7`**: os cinco primeiros já estavam lembrados.

A dupla exibição da primeira versão é o sintoma visível de trabalho recomputado — exatamente o que a memoização elimina.
...
<Solution exercise="3.52">

**Sem memoização** (verificado executando — experimente!):

| Após | `sum` |
|---|---|
| `let sum = 0;` | 0 |
| `const seq = …` | **1** (a cabeça é avaliada já) |
| `const y = …` | **6** (o filtro avança até o primeiro par: 1, 3, 6) |
| `const z = …` | **15** (z percorre sua PRÓPRIA cópia de seq — accum roda de novo com o sum atual!) |
| `stream_ref(y, 7)` | **162**, e o eco é `162` |
| `display_stream(z)` | exibe `15, 180, 230, 305` e `sum` termina em **362** |

Os valores estranhos vêm de `accum` ser executada **mais de uma vez por elemento**: cada percurso não-memoizado de `seq` reaplica `accum` sobre o `sum` corrente, então `y` e `z` veem "sequências" diferentes — e mudam um ao outro pelo caminho.

**Com `memo` em cada cauda**, cada `accum` roda exatamente uma vez e `seq` é de fato a sequência de somas parciais 1, 3, 6, 10, 15, …, 210:

| Após | `sum` |
|---|---|
| `const seq` | 1 |
| `const y` | 6 |
| `const z` | **10** (reaproveita o prefixo já materializado) |
| `stream_ref(y, 7)` | eco `136`, `sum` = 136 |
| `display_stream(z)` | `10, 15, 45, 55, 105, 120, 190, 210`, `sum` = **210** |

**A moral:** misturar streams com funções que têm *efeitos colaterais* é perigoso — o número de vezes que o efeito acontece depende da estratégia de avaliação, não do programa "visível". É o aviso que fecha esta seção do livro.
