...
<CodePlayground
  ...
  checks={[{expression: 'a(10)', expected: '15'}, {expression: 'a(10)', expected: '25'}]}
  height={150}
  showLineNumbers={false}
/>
...
**Ideia.** O próprio parâmetro `sum` serve de variável de estado: a função devolvida fecha sobre ele e o atualiza a cada chamada.

<CodePlayground
  code={`function make_accumulator(sum) {
    return amount => {
        sum = sum + amount;
        return sum;
    };
...
Cada chamada a `make_accumulator` cria um ambiente novo com seu próprio `sum` — dois acumuladores nunca interferem um no outro. (Cole a definição no bloco do exercício acima e veja as verificações passarem.)
...
<CodePlayground
  ...
  checks={[{expression: 's(100)', expected: '10'}, {expression: 's("how many calls")', expected: '1'}]}
  height={150}
  showLineNumbers={false}
/>
...
<CodePlayground
  code={`function make_monitored(f) {
    let count = 0;
    function mf(m) {
        if (m === "how many calls") {
            return count;
...
**Por que funciona.** `count` vive no ambiente criado pela chamada de `make_monitored`; `mf` fecha sobre ele. As strings especiais são interceptadas *antes* de chegar a `f` — repare que perguntar `"how many calls"` não conta como chamada.
...
<CodePlayground
  ...
  checks={[{expression: 'acc("secret password", "withdraw")(40)', expected: '60'}, {expression: 'acc("some other password", "deposit")(50)', expected: '"Incorrect password"'}]}
  height={150}
  showLineNumbers={false}
/>
...
**Ideia.** `dispatch` ganha um primeiro argumento de senha; a checagem vem antes de qualquer outra coisa. Para manter o contrato (o resultado de `acc(...)` é sempre uma função a ser aplicada à quantia), a recusa também é uma função — que ignora a quantia e devolve a reclamação.

<CodePlayground
  code={`function make_account(balance, password) {
    function withdraw(amount) {
        if (balance >= amount) {
            balance = balance - amount;
            return balance;
...
**Ideia.** Mais uma variável de estado local, `consecutive_wrong`, zerada a cada acesso correto.

<CodePlayground
  code={`function call_the_cops() {
    return "☎ Chamando a polícia!";
}
...
const acc = make_account(100, "senha");
let i = 0;
while (i < 8) {
    display(acc("errada", "withdraw")(10));
    i = i + 1;
}
acc("senha", "withdraw")(10);`}
  height={560}
  showLineNumbers={false}
/>

Na oitava tentativa errada consecutiva, a polícia é chamada; um acesso correto zera o contador e a conta volta ao normal.
