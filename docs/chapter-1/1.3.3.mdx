...
Por exemplo, podemos usar este método para aproximar o ponto fixo do cosseno, começando com 1 como palpite inicial:<a name="footnote-link-3"></a>[<sup>3</sup>](#footnote-3)
...
**A demonstração.** $\phi$ satisfaz $\phi^2 = \phi + 1$ (é a raiz positiva dessa equação, por definição). Dividindo os dois lados por $\phi$: $\phi = 1 + 1/\phi$ — ou seja, $\phi$ é exatamente um ponto fixo de $x \mapsto 1 + 1/x$.

<CodePlayground
  code={`fixed_point(x => 1 + 1 / x, 1);`}
  hiddenCode={`const tolerance = 0.00001;
function fixed_point(f, first_guess) {
    function close_enough(x, y) {
...
O resultado, $\approx 1{,}6180$, é a razão áurea.
...
<Solution exercise="1.36">

**Versão com impressão** e a busca por $x^x = 1000$:
...
— converge em **cerca de 9 passos** para $x \approx 4{,}5555$. O amortecimento não muda o ponto fixo (a média de $x$ com $f(x)$ coincide com $x$ exatamente quando $f(x) = x$), só doma a oscilação.
...
<Solution exercise="1.37">

**a. Recursivo** — contando do termo $k$ para dentro:

<CodePlayground
  code={`function cont_frac(n, d, k) {
    function frac(i) {
        return i > k
               ? 0
               : n(i) / (d(i) + frac(i + 1));
...
cont_frac(i => 1, i => 1, 12);`}
  height={260}
  showLineNumbers={false}
/>

Com $1/\phi \approx 0{,}6180$, a precisão de 4 casas decimais chega com **k = 11 ou 12**.

**b. Iterativo** — construindo de trás para frente, do termo $k$ até o 1:
...
<Solution exercise="1.38">

**Ideia.** Só precisamos descrever a sequência de denominadores $1, 2, 1, 1, 4, 1, 1, 6, \ldots$: nas posições $2, 5, 8, \ldots$ (isto é, $i \equiv 2 \pmod 3$) o valor é $\frac{2(i+1)}{3}$; nas demais é 1.
...
2 + cont_frac(i => 1, euler_d, 20);`}
  height={340}
  showLineNumbers={false}
/>

Com 20 termos o resultado já é $e \approx 2{,}718281828$ com todas as casas mostradas corretas.
...
<Solution exercise="1.39">

**Ideia.** Na fórmula de Lambert, $N_1 = x$ e os demais $N_i = -x^2$ (o sinal de menos da fórmula vira sinal do numerador); os denominadores são os ímpares $1, 3, 5, \ldots$
...
Com $k = 10$, `tan_cf(1, 10)` coincide com `math_tan(1)` $\approx 1{,}5574$ em todas as casas exibidas — a fração de Lambert converge rápido.
