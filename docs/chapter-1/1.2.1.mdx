...
**Primeira versão** — `inc(plus(dec(a), b))`. Pelo modelo de substituição:
...
A cadeia de `inc`s adiados cresce e depois se contrai: é um **processo recursivo** (linear).

**Segunda versão** — `plus(dec(a), inc(b))`:
...
Todo o estado vive nos argumentos; nada fica adiado: é um **processo iterativo** (linear).

Você pode *ver* a diferença com o rastreador. Na versão recursiva, cada nível retorna um valor intermediário diferente (ainda há trabalho a fazer na volta); na iterativa, todos os níveis retornam o mesmo `9` — a resposta já está pronta quando o fundo é atingido:
...
Troque o corpo por `plus(dec(a), inc(b))`, execute de novo e compare as duas árvores.
...
**As funções.** Para $n$ inteiro positivo:

- `f(n)` = `A(0, n)` calcula $2n$ — é o caso `x === 0`.
- `g(n)` = `A(1, n)` calcula $2^n$: `A(1, n) = A(0, A(1, n-1)) = 2 · A(1, n-1)`, com `A(1, 1) = 2`.
- `h(n)` = `A(2, n)` calcula a **torre de expoentes** $2^{2^{\cdot^{\cdot^{2}}}}$ com $n$ dois — a *tetração* $2 \uparrow\uparrow n$: `A(2, n) = A(1, A(2, n-1)) = 2^{A(2, n-1)}`, com `A(2, 1) = 2`. Assim `h(1) = 2`, `h(2) = 4`, `h(3) = 16`, `h(4) = 65536`, e `h(5)` já é $2^{65536}$ — um número de quase vinte mil dígitos.

**Armadilha comum.** Tentar `A(3, 4)` ou `h(5)` no playground: o valor não cabe em nenhum lugar razoável — a função de Ackermann cresce mais rápido que qualquer torre fixa de expoentes.
