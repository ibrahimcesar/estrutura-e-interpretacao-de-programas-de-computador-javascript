...
- 1º teste: avalia `40 % ...`? Não — `b` é `40`, ainda sem resto. Testes seguintes avaliam expressões cada vez maiores: **1 + 2 + 4 + 7 = 14 avaliações de `%` nos predicados**, até o teste dar verdadeiro.
- No final, o `a` retornado é `206 % 40 % ...` acumulado, custando mais **4** avaliações.
