...
que converge para $\pi/8$ (muito lentamente):<a name="footnote-link-1"></a>[<sup>1</sup>](#footnote-1)
...
<Solution exercise="1.29">

**Ideia.** O coeficiente de $y_k$ é 1 nas pontas, 4 nos índices ímpares e 2 nos pares — então o termo da soma pode decidir o coeficiente olhando a paridade de $k$, e `sum` faz o resto.
...
**Comparação.** O valor exato de $\int_0^1 x^3\,dx$ é $0{,}25$. A `integral` do texto com $dx = 0{,}01$ dá $\approx 0{,}24998$; a Regra de Simpson dá **exatamente 0,25** já com $n = 100$ — para polinômios de grau até 3, Simpson não é só mais precisa, é exata (a menos de erros de arredondamento de ponto flutuante).
...
function wallis_pi(n) {
    function term(k) {
        const numerator = k % 2 === 0 ? k + 2 : k + 1;
        const denominator = k % 2 === 0 ? k + 1 : k + 2;
        return numerator / denominator;
    }
    return 4 * product(term, 1, inc, n);
}
...
O termo de índice $k$ é $\frac{k+2}{k+1}$ para $k$ par e $\frac{k+1}{k+2}$ para $k$ ímpar — exatamente o padrão $\frac{2}{3}\cdot\frac{4}{3}\cdot\frac{4}{5}\cdots$. Com $n = 1000$, `wallis_pi` dá $\approx 3{,}1400$ — a série converge devagar.
...
accumulate((x, y) => x + y, 0, identity, 1, inc, 10);`}
  height={330}
  showLineNumbers={false}
/>
...
<Solution exercise="1.33">

**A abstração:** igual a `accumulate`, mas termos reprovados no filtro entram como se não existissem.

<CodePlayground
  code={`function filtered_accumulate(combiner, null_value, term,
                             a, next, b, filter) {
    return a > b
           ? null_value
           : filter(a)
           ? combiner(term(a),
                      filtered_accumulate(combiner, null_value, term,
...
// a. soma dos quadrados dos primos em [a, b]
function sum_of_prime_squares(a, b) {
    return filtered_accumulate((x, y) => x + y, 0,
                               square, a, inc, b, is_prime);
}

// b. produto dos i < n com gcd(i, n) = 1
function gcd(a, b) {
    return b === 0 ? a : gcd(b, a % b);
}
function product_of_coprimes(n) {
    return filtered_accumulate((x, y) => x * y, 1,
                               identity, 1, inc, n - 1,
                               i => gcd(i, n) === 1);
}

display(sum_of_prime_squares(2, 10)); // 4 + 9 + 25 + 49
product_of_coprimes(10);              // 1 * 3 * 7 * 9`}
  hiddenCode={`function smallest_divisor(n) {
...
**Verificação.** `sum_of_prime_squares(2, 10)` = $4 + 9 + 25 + 49 = 87$; `product_of_coprimes(10)` = $1 \cdot 3 \cdot 7 \cdot 9 = 189$.
