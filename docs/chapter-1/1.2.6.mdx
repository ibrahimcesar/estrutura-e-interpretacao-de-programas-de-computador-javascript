...
<Solution exercise="1.21">

<CodePlayground
  code={`display(smallest_divisor(199));
...
**Resultados:** `199` e `1999` são primos (o menor divisor é o próprio número); já `19999 = 7 × 2857`, então seu menor divisor é **7**.
...
<Solution exercise="1.22">

**Ideia.** Caminhar só pelos ímpares, decrementando o contador a cada primo encontrado.
...
**Os primos:** 1009, 1013, 1019; 10007, 10009, 10037; 100003, 100019, 100043; 1000003, 1000033, 1000037.

**Sobre os tempos.** Em máquinas atuais, todos esses testes levam praticamente 0 ms — o exercício foi escrito quando computadores eram milhões de vezes mais lentos. Para *ver* a razão $\sqrt{10} \approx 3{,}16$, procure primos perto de $10^{10}$, $10^{11}$ e $10^{12}$ (ainda dentro da precisão dos números de JavaScript, $2^{53}$): as razões entre tempos consecutivos se aproximam de $\sqrt{10}$, confirmando que o tempo de execução acompanha o número de passos, $\Theta(\sqrt n)$.
...
<Solution exercise="1.23">

<CodePlayground
  code={`function next(n) {
    return n === 2 ? 3 : n + 2;
}
function find_divisor(n, test_divisor) {
    return square(test_divisor) > n
...
**A expectativa se confirma?** Só em parte. Metade dos candidatos é testada, mas cada passo ficou mais caro: `next` é uma chamada de função com um teste condicional, no lugar de uma soma direta. Na prática a razão observada fica tipicamente entre 1,5 e 2 — o número de *passos* caiu pela metade, mas passos não têm todos o mesmo custo. É um bom lembrete de que $\Theta$ esconde constantes que a medição revela.
...
**Ideia.** Trocar `is_prime` por `fast_is_prime` (teste de Fermat) na mesma estrutura de medição:
...
**Expectativa.** Com crescimento $\Theta(\log n)$, testar primos perto de $1.000.000$ deveria custar cerca de **o dobro** de testar perto de $1.000$ ($\log 10^6 = 2 \log 10^3$). Na prática a razão medida costuma ficar perto disso, mas não exatamente: o custo de cada multiplicação não é constante, o número de repetições do teste e o gerador aleatório adicionam ruído, e para números tão pequenos o tempo é dominado por sobrecustos fixos do interpretador.
...
<Solution exercise="1.25">

**Alyssa está errada** — a função dela é matematicamente correta, mas inútil na prática.

`expmod` reduz módulo `m` **a cada passo**, então os números intermediários nunca passam de $m^2$. A versão de Alyssa calcula primeiro $b^e$ *por inteiro*: para um teste de Fermat com $n$ perto de um milhão, isso é um número com milhões de dígitos. Em uma linguagem com inteiros exatos, isso torna cada multiplicação lentíssima; em JavaScript é pior — números perdem precisão acima de $2^{53}$ e viram `Infinity` por volta de $10^{308}$, então o resultado do `%` final é simplesmente **errado**. Experimente: `fast_expt(999999, 999999)` é `Infinity`, e `Infinity % m` é `NaN`.

A lição é a da nota de rodapé 3: reduzir módulo $m$ cedo e sempre não é uma otimização opcional — é o que mantém o cálculo dentro do mundo dos números representáveis.
...
<Solution exercise="1.26">

Na versão de Louis, o argumento de `square` virou **duas chamadas idênticas**: cada nível de recursão com expoente par dispara *duas* subárvores completas em vez de uma. A recursão que antes era uma *corrente* de comprimento $\log n$ virou uma *árvore binária* de profundidade $\log n$ — e uma árvore binária com essa profundidade tem da ordem de $2^{\log_2 n} = n$ nós.

Formalmente: $T(e) = 2\,T(e/2) + \Theta(1)$ resolve para $T(e) = \Theta(e)$, enquanto com `square` era $T(e) = T(e/2) + \Theta(1) = \Theta(\log e)$. A avaliação em ordem aplicativa avalia o operando **uma vez** quando ele é argumento de `square`, mas avalia **as duas ocorrências** quando Louis as escreve explicitamente.
...
**Ideia.** Testar a congruência $a^n \equiv a \pmod n$ para *todos* os $a < n$:
...
display(fools_fermat(561));
display(fools_fermat(1105));
display(fools_fermat(1729));
fools_fermat(15);`}
  hiddenCode={`function square(x) {
    return x * x;
}
...
**Resultado.** Os números de Carmichael 561, 1105 e 1729 passam no teste para *todas* as bases — indistinguíveis de primos aos olhos de Fermat — enquanto um composto comum como 15 falha imediatamente. (O 1729 é o famoso "número de táxi" de Hardy e Ramanujan.)
...
<Solution exercise="1.28">

**Ideia.** No passo de elevação ao quadrado, sinalizar (retornando 0) quando encontramos uma raiz quadrada não trivial de 1 módulo $n$ — um $x \neq 1, n-1$ com $x^2 \equiv 1 \pmod n$:

<CodePlayground
  code={`function square_check(x, m) {
    return x !== 1 && x !== m - 1 && (x * x) % m === 1
           ? 0
           : (x * x) % m;
}
function expmod(base, exp, m) {
    return exp === 0
           ? 1
           : is_even(exp)
           ? square_check(expmod(base, exp / 2, m), m)
           : (base * expmod(base, exp - 1, m)) % m;
}
...
display(fast_is_prime(1009, 20));   // primo
display(fast_is_prime(561, 20));    // Carmichael!
display(fast_is_prime(6601, 20));   // Carmichael!
fast_is_prime(1000037, 20);`}
  hiddenCode={`function is_even(n) {
    return n % 2 === 0;
}`}
  height={480}
...
**Por que funciona.** O 0 é "pegajoso": uma vez sinalizado, propaga-se até o topo (qualquer coisa ao quadrado ou multiplicada módulo $m$ continua 0) e faz `expmod(a, n-1, n) === 1` falhar. Note os números de Carmichael que enganavam Fermat no exercício 1.27 sendo desmascarados — como pelo menos metade das bases revela uma raiz não trivial quando $n$ é composto, vinte rodadas dão confiança de mais de $1 - 2^{-20}$.
