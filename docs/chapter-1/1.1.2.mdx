...
O interpretador JavaScript precisa executar a declaração da constante `size` antes que o nome `size` possa ser usado em uma expressão. Nesta edição, os exemplos de uma mesma página compartilham o mesmo ambiente: ao executar um bloco, as instruções dos blocos anteriores são avaliadas primeiro, automaticamente. Assim, executar
...
equivale a executar o programa completo, com suas dependências incluídas:
...
<CodePlayground
  code={`const circumference = 2 * pi * radius;`}
  session="1.1.2"
  height={150}
  showLineNumbers={false}
/>
...
<a name="footnote-link-1"></a> [1] Neste livro, não mostramos a resposta do interpretador para avaliar programas que terminam com declarações, uma vez que isso pode depender de instruções anteriores. Consulte o exercício [4.8](/chapter-4/4.1.2#ex-4.8) para obter detalhes.
