...
Todos os demais geradores apenas conduzem `comp_env` intacto. O ambiente de tempo de compilação é a *sombra estática* do ambiente de execução: cresce nos mesmos pontos, mas carrega só os **nomes** — o suficiente para o exercício 5.44 calcular endereços.
...
**b. Dobra de constantes** (constant folding): com a propagação instalada, operações abertas (exercício 5.41) cujos operandos são constantes conhecidas podem ser avaliadas **pelo compilador**: `compile_open_code` testa se ambos os lados têm valor conhecido e, em caso positivo, emite `assign(target, constant(resultado))` — `2 * 3` no fonte não gera multiplicação nenhuma no objeto.
