...
<Solution exercise="5.27">

Rodando `factorial(n)` iterativa para vários `n` com a pilha monitorada, a profundidade máxima é **constante** (a mesma para todo `n` — tipicamente algo como 10, dependendo dos detalhes do avaliador), confirmando a recursão de cauda: a iteração inteira acontece sem crescimento de pilha.

O número **total** de pushes cresce linearmente: os dados ajustam-se a uma reta $a \cdot n + b$ (com $a$ na casa de poucas dezenas — cada iteração custa um número fixo de operações de pilha do avaliador). As duas constantes pedidas são o custo por iteração e o custo fixo de entrada/saída.

</Solution>

### Exercício 5.28

Para comparação com o exercício 5.27, explore o comportamento da seguinte função para computar fatoriais recursivamente:
...
<Solution exercise="5.28">

Agora a profundidade máxima também cresce **linearmente** com `n`: cada chamada recursiva pendente (`n * factorial(n - 1)` tem a multiplicação esperando) retém seu quadro na pilha até a volta. Os dados ajustam-se a retas tanto para o total de pushes quanto para a profundidade — e é a *profundidade* que distingue os processos: constante na versão iterativa (exercício 5.27), linear na recursiva, exatamente a análise do capítulo 1 medida em hardware simulado.
...
Com o `ev_return` modificado (avaliando a expressão de retorno **antes** de reverter a pilha, com um `save`/`go_to`/marcador a mais), toda chamada — em posição de cauda ou não — retém estado na pilha. Repetindo os experimentos: as **duas** versões de `factorial` passam a mostrar profundidade máxima linear em `n`; a versão "iterativa" perde sua vantagem de espaço e só conserva a diferença de constantes.

É a demonstração de que a recursão de cauda não é um truque do compilador de JavaScript, e sim uma **decisão do avaliador** sobre quando desmontar a pilha — mova a reversão para depois da avaliação e a propriedade evapora.
...
<Solution exercise="5.30">

Medindo `fib(n)` para $n$ crescente com a pilha monitorada:

- **Profundidade máxima**: cresce **linearmente** em $n$ (a espinha mais funda da árvore de chamadas) — os dados ajustam-se a uma reta $a\,n + b$.
- **Total de pushes**: cresce como **Fib(n)** — exponencialmente. A razão entre os totais de $n$ e $n-1$ tende à razão áurea; o ajuste pedido tem a forma $S(n) = k\,\text{Fib}(n+1) + c$ para constantes $k$ e $c$ que os dados determinam (o número de pushes por nó da árvore é fixo).

Espaço linear, tempo exponencial — a assinatura da recursão em árvore, agora medida em operações de pilha reais.
...
<Solution exercise="5.31">

**a. Nomes não vinculados.** `lookup_symbol_value` passa a devolver um código de condição único (um par alocado uma vez, como o marcador do exercício 5.23) em vez de sinalizar erro no JavaScript hospedeiro. Todo ponto do controlador que faz `assign("val", op("lookup_symbol_value"), …)` precisa, em seguida, de `test`/`branch` para `signal_error` com a mensagem apropriada — é "muito trabalho" porque a busca aparece em vários trechos (`ev_name`, atribuição, aplicação), e cada um precisa do seu teste.

**b. Erros de primitivas** (divisão por zero, `head` de não-par…): a mesma técnica um nível abaixo — `apply_primitive_function` embrulha a chamada e devolve o código de condição em vez de deixar o sistema hospedeiro explodir; `primitive_apply` testa e desvia para `signal_error`.

O padrão geral: transformar *exceções do hospedeiro* em *valores testáveis da máquina* — que é literalmente o que hardware real faz com códigos de condição e flags, e o que sistemas de exceção de linguagens de verdade automatizam por baixo dos panos.
...
[^2]: Há outros erros que gostaríamos que o interpretador tratasse, mas estes não são tão simples. Veja exercício 5.31.
