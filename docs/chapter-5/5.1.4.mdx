...
**a. Exponenciação recursiva** — precisa de pilha e `continue`, como a máquina de fatorial do texto:
...
(Só `continue` precisa ser salvo: `b` não muda e `n` não é usado na volta.) Os caminhos de dados incluem a pilha, o decrementador de `n` e o multiplicador `b · val`.

**b. Exponenciação iterativa** — sem pilha; o estado vive em `counter` e `product`:
...
"expt_iter_loop",
   ...
   branch(label("expt_iter_done")),
   assign("counter", list(op("-"), reg("counter"), constant(1))),
   assign("product", list(op("*"), reg("b"), reg("product"))),
   go_to(label("expt_iter_loop")),
"expt_iter_done"
```

Lado a lado, as duas máquinas *materializam* a distinção do capítulo 1: o processo recursivo precisa de memória auxiliar crescente (a pilha); o iterativo, de um número fixo de registradores.
...
**Fatorial de 3** (máquina recursiva do texto). A pilha guarda pares (`continue`, `n`) a caminho da base e os consome na volta:

```
chamada        pilha (topo à esquerda)          ação
fact(3)        —                                salva continue=done, n=3
fact(2)        [done, 3]                        salva continue=after, n=2
fact(1)        [after, 2, done, 3]              caso base: val ← 1
volta n=2      [done, 3]                        val ← 2 · 1 = 2
volta n=3      —                                val ← 3 · 2 = 6
```

Profundidade máxima da pilha: 4 itens (dois pares).

**Fibonacci de 3**:

```
fib(3): salva continue, n → calcula fib(2)
  fib(2): salva → calcula fib(1) = 1
          restaura; salva n (com fib(1) em val) → calcula fib(0) = 0
          restaura; val ← 1 + 0 = 1
restaura; salva val=1, n → calcula fib(1) = 1
restaura; val ← 1 + 1 = 2
```

O que observar: em Fibonacci a pilha guarda, além de `continue`/`n`, o **resultado parcial** de Fib(n−1) enquanto Fib(n−2) é computado — a "operação adiada" da recursão em árvore aparecendo como conteúdo concreto de pilha.
...
Estão no trecho `afterfib_n_1`: ao voltar do cálculo de Fib(n−1), o controlador faz `restore("continue")` e, logo em seguida — sem que `continue` tenha sido usado ou alterado — `save("continue")` de novo, antes de partir para Fib(n−2).

Como o valor restaurado é imediatamente salvo de volta, o par `restore`/`save` é um não-op: pode ser removido, deixando o valor simplesmente **na pilha** durante o cálculo de Fib(n−2). A máquina fica mais rápida — duas operações de pilha a menos por nó de uma árvore de chamadas com um número exponencial de nós.
