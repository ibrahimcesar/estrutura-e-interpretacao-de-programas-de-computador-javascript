...
controller(
  list(
    ...
    assign("product",
           list(op("*"), reg("counter"), reg("product"))),
    assign("counter",
           list(op("+"), reg("counter"), constant(1))),
    go_to(label("test_counter")),
    "fact_done"))
```

Tradução direta do laço do exercício 5.1: o teste decide a saída, as duas atribuições são o corpo, e o `go_to` fecha o ciclo.
