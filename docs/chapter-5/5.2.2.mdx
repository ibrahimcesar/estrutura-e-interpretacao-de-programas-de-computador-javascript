...
Com o montador como está, `extract_labels` constrói a lista de rótulos na ordem do texto e a busca encontra a **primeira** ocorrência — então `go_to(label("here"))` salta para o primeiro `here`, e `a` termina valendo **3**.

Para rejeitar a ambiguidade, `extract_labels` verifica duplicatas ao inserir cada rótulo:

```javascript
is_undefined(assoc(next_element, labels))
? pair(make_label_entry(next_element, insts), labels)
: error(next_element, "rótulo duplicado -- assemble")
```

O montador falha na montagem — em tempo de "compilação" da máquina, não no meio da execução, que é onde se quer pegar esse tipo de erro.
