...
Sem o `return`, o valor da chamada recursiva de `count` é calculado e **descartado** — mas o avaliador só descobre isso *depois* de voltar. Cada nível empilha o que precisa para continuar a sequência após a chamada (o `continue`, o ambiente da iteração) e nada disso pode ser liberado antes de a recursão inteira descer e subir: a pilha cresce linearmente com o número de iterações, mesmo sendo a função "iterativa" na aparência.

Com `return`, `ev_return` desmonta a pilha até o marcador **antes** de avaliar a expressão de retorno — a chamada recursiva acontece com a pilha já limpa, e o processo roda em espaço constante. A lição da seção em miniatura: em JavaScript, a posição de cauda é criada pelo `return`; sem ele, não há chamada de cauda para otimizar.
...
Com um marcador impossível de confundir com dados do usuário:

```javascript
const marker = list("marker");   // identidade única: novo par, === só a si

// push_marker_to_stack ≡ em compound_apply:
save_val_of(marker)              // isto é: empilhar o marcador

// revert_stack_to_marker ≡ em ev_return / return_undefined:
"revert_loop",
    restore("val"),                       // tira o topo (para QUALQUER registrador)
    test(list(op("==="), reg("val"), constant(marker))),
    branch(label("reverted")),
    go_to(label("revert_loop")),
"reverted",
    …
```

O laço restaura repetidamente para um registrador de rascunho até encontrar o marcador — e é por isso que a dica menciona o exercício 5.11: essa disciplina **exige** a pilha permissiva (restaurar para registrador diferente do salvo), pois os valores descartados pertenciam a registradores variados. O `===` funciona como teste de identidade porque `list("marker")` aloca um par novo que nenhum outro valor pode ser.

</Solution>

### Exercício 5.24

Implemente `push_marker_to_stack` e `revert_stack_to_marker` como instruções de máquina de registradores, seguindo a implementação de `save` e `restore` na seção 5.2.1. Adicione funções `push_marker` e `pop_marker` para acessar pilhas, espelhando a implementação de `push` e `pop` na seção 5.2.1. Note que você não precisa realmente inserir um marcador na pilha. Em vez disso, você pode adicionar uma variável de estado local ao modelo de pilha para rastrear a posição do último `save` antes de cada `push_marker_to_stack`. Se você optar por colocar um marcador na pilha, veja a dica no exercício 5.23.
<Solution exercise="5.24">

A implementação sem marcador físico: a pilha guarda, além dos itens, **a posição do último marcador**:

```javascript
function make_stack() {
    let stack = null;
    let marks = null;                    // pilha de posições marcadas
    function push(x) { stack = pair(x, stack); }
    function pop() { … como antes … }
    function push_marker() {
        marks = pair(stack, marks);      // lembra o "nível" atual
    }
    function pop_marker() {
        stack = head(marks);             // volta ao nível lembrado
        marks = tail(marks);
    }
    …
}
...
`push_marker` anota o próprio ponteiro da lista `stack` (a posição); `pop_marker` restaura esse ponteiro, descartando de uma vez tudo que foi empilhado desde então — **O(1)**, sem laço. As instruções de máquina `push_marker_to_stack`/`revert_stack_to_marker` viram chamadas diretas dessas operações no montador, com o mesmo formato de `make_save`/`make_restore`. Comparado ao exercício 5.23: mesma semântica, sem varrer a pilha e sem valor sentinela circulando pelos registradores.
...
[^6]: As instruções especiais `push_marker_to_stack` e `revert_stack_to_marker` não são estritamente necessárias e poderiam ser implementadas explicitamente empurrando e removendo um valor marcador de e para a pilha. Qualquer coisa que não possa ser confundida com um valor no programa pode ser usada como marcador. Veja exercício 5.23.
