...
<Solution exercise="5.36">

A diferença da versão do texto: o condicional testa `n === 1` como *expressão de retorno* de outra forma (ou a base retorna direto), mudando onde o compilador emite os desvios. Comparando as saídas compiladas, as instruções são **essencialmente as mesmas** — mudam os pontos onde o teste acontece e qual ramo cai no `goto` — e a moral é que variações superficiais do fonte produzem variações igualmente superficiais do objeto: o esqueleto (teste, chamada recursiva com `save`/`restore` mínimos via `preserving`, multiplicação na volta) é idêntico, porque é a *estrutura recursiva*, não a sintaxe, que dita o código.
...
<Solution exercise="5.37">

O ponto a observar na saída da compilação da fatorial **iterativa**: o laço `iter` vira uma chamada em posição de cauda — o compilador emite, na chamada recursiva de `iter`, um salto direto (`compile` com ligação `"return"`) **sem nenhum `save` pendente**: a pilha não cresce entre iterações. Compare com a fatorial recursiva do texto, cujo código objeto salva `continue`/`env` antes da chamada e multiplica na volta.

O código compilado da iterativa é, na prática, o laço da máquina de registradores do exercício 5.1 — teste, duas atualizações, salto — redescoberto mecanicamente pelo compilador a partir do fonte funcional. Processos iterativos compilam para iteração de verdade; é a recursão de cauda fechando o ciclo do livro.
...
<Solution exercise="5.38">

Lendo a figura de trás para diante: o código monta uma função com dois parâmetros que, no corpo, testa `x === 0`? não — os acessos são `lookup "x"` e `lookup "n"`?… O método é o que importa: (1) identifique os `lookup_symbol_value` para descobrir os nomes envolvidos; (2) ache os testes (`is_falsy`) e o que cada ramo devolve; (3) reconstrua o condicional e as chamadas pelos `save`/`restore` e ligações de `continue`. O programa da figura 5.18 é a declaração recursiva
...
— a fatorial com o nome trocado (`f`), reconhecível pelo literal 1 no ramo de base, pela multiplicação pendente após a chamada recursiva e pelo decremento do argumento na montagem de `argl`.
...
<Solution exercise="5.39">

O compilador do texto gera a avaliação dos argumentos **da direita para a esquerda**: `construct_arglist` inverte a lista de códigos de operandos e vai fazendo `pair` do valor corrente com `argl` — o último operando é avaliado primeiro (assim `argl` é construída por `pair` sem inversão final).

**Onde**: em `construct_arglist`, na linha que faz `reverse(operand_codes)`.

**Para mudar** para esquerda→direita: processe os códigos na ordem original acumulando em `argl` com um *append* no fim (`argl ← append(argl, list(val))`), ou construa como agora e acrescente uma inversão final de `argl`. **O custo**: ou um `append` linear por argumento (quadrático no total), ou uma `reverse` extra em tempo de execução por aplicação — a ordem direita→esquerda existe justamente porque `pair` constrói listas de trás para frente de graça. (O avaliador de controle explícito da seção 5.4 pagou o mesmo preço na outra moeda.) Para aderir à especificação ECMAScript, o preço da inversão final é o mais honesto.
...
<Solution exercise="5.40">

Trocando `preserving` por uma versão que sempre embrulha:
    ...
    return is_null(regs)
           ...
           : preserving(tail(regs),
                 make_instruction_sequence(…,
                     append(list(save(head(regs))),
                            append(statements(seq1),
                                   list(restore(head(regs)))))),
                 seq2);
}
```

Compilando `f("x", "y")` com isso: aparecem `save("env")`/`restore("env")` em volta da avaliação do nome `f` (que não toca `env`), `save("argl")` em volta de literais (que não tocam `argl`), `save("fun")`/`restore("fun")` em volta da montagem de argumentos constantes — meia dúzia de pares de operações de pilha para uma aplicação que, com o `preserving` inteligente, compila **sem nenhuma**. A comparação mostra o mecanismo pelo avesso: `preserving` não é uma otimização exótica, é só *não pagar por proteção que ninguém pediu* — as anotações de "registradores que modifica/precisa" de cada sequência são o que torna a decisão local e segura.
...
<Solution exercise="5.41">

Codificação aberta das primitivas: um caso novo no compilador reconhece aplicações cujo operador é um dos operadores conhecidos (`+`, `-`, `*`, `/`, `===`, …) e emite a operação de máquina diretamente:

```javascript
// em compile, antes do caso geral de aplicação:
: is_operator_combination_open_codable(comp)
? compile_open_code(comp, target, linkage, env)
: …

function compile_open_code(comp, target, linkage) {
    const a = compile(first_operand(comp), "arg1", "next");
    const b = compile(second_operand(comp), "arg2", "next");
        ...
        preserving(list("env"),
            ...
            preserving(list("arg1"),
                ...
                make_instruction_sequence(list("arg1", "arg2"), list(target),
                    list(assign(target,
                         list(op(operator_of(comp)),
                              reg("arg1"), reg("arg2"))))))));
}
```

`a + 1` compila para dois `assign` e uma soma — sem `argl`, sem `lookup` de `"+"`, sem teste primitiva/composta, sem desvios. A ressalva clássica: isso assume que `+` **é** a primitiva — se o programa do usuário redefinir `+` (possível com a variante do exercício 4.5), a codificação aberta fica incorreta; compiladores reais guardam essa otimização atrás da suposição (ou verificação) de que as primitivas não foram sombreadas.
