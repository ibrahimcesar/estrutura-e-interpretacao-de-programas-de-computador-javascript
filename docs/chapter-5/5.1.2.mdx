...
**Primeira versão** — tratando `is_good_enough` e `improve` como operações primitivas: registradores `x` e `guess`; o controlador é um laço puro:

```
   guess ← 1
test
   se good_enough(guess, x), vá para done
   guess ← improve(guess, x)
   vá para test
done
```

**Versão expandida** — abrindo as duas "primitivas" em aritmética de verdade (quadrado, subtração, valor absoluto, comparação com 0.001; divisão e média), o teste vira uma sequência com um registrador temporário:
...
test
   ...
   t ← t − x
   t ← abs(t)
   se t < 0.001, vá para done
   t ← x / guess
   guess ← (guess + t) / 2
   vá para test
```

Os caminhos de dados ganham os operadores aritméticos e o registrador `t`. A comparação das duas versões é o ponto do exercício: o *nível de abstração* das operações primitivas é uma escolha de projeto — descer de nível troca simplicidade do controlador por realismo do hardware.
